...
- 🛠️ Smooth app-scroll (`smooth_scroll_apps`) left a blank band above/below the text that grew with scroll speed, and stepped one line at a time before easing. (20260703)
	...
	- ◐ Residual band jitter during a slide (nano; "almost perfect" otherwise). Two symptoms, different causes:
		- Text moving UP (content scrolls up): the drop-shadow *under* the inverse-video header title jumps DOWN.
			- Root cause: the readability glow was painting a bg-coloured halo around the header's glyphs; below the bar that halo is a patch over whatever's behind the text area (a bg image), i.e. the "drop-shadow", and part of it rode the sliding middle region so it jumped on each step.
			- Fixed (shared with the inverse-thin bug below): the glow no longer applies over any cell that carries its own solid bg (reverse video, coloured bg, selection) - they already have full contrast, so the halo there was pure artifact. The bgcolor map's alpha is now an own-bg mask; the blur gates each tap by it, so an inverse header casts no halo anywhere - nothing left to jump. Verified by an A/B render: below a reverse bar over a bright image the halo patch (216,216,225) is gone (235,235,245 = the image) while all other text glow is byte-identical.
		- Text moving DOWN fast: the bottom two lines jump UP (can't tell if their drop-shadow does too). STILL OPEN - separate cause, not the glow.
			- Only shows up "fast enough", which points at re-detection mid-ease: a new step re-captures the band sizes and *sets* `app_off` to a fresh full step, so if the bottom-band count differs for one frame (or `app_off` jumps), the band row shifts by a line for that frame.
			- Likely fix: hold the band sizes stable across an in-progress ease (don't shrink a band mid-slide). Feel-sensitive - verify on hardware, gated by `smooth_scroll_apps`.
