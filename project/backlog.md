...
- ✅ A pipeline run aborted at the release stage and reported an application problem, when the compiler had crashed twice in a row. (20260804)
	...
	- Open question, and the reason the count is a setting: whether this is the compiler or this machine's memory. Repeated faults at different points on identical input mean something varies between runs, which is either a latent defect in the optimizer or marginal hardware. Nothing inside the running system can separate the two - this board exposes no memory error counters at all - so it would take a standalone memory test to clear the hardware. Worth doing if the rate keeps climbing: it has gone from occasional to several times in a day.
...
- ✅ Wallpaper settings renamed, and given two master switches.
	...
	- Verified: an existing config renames in one launch and keeps every value; a folder under `~` rotates; each switch changes what renders.
