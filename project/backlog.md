...
- [✔️] Local CI/CD pipeline, one command, fail-fast, reusable across projects (`cicd/`). (20260628)
	- Status: Done. `cicd/cicd.bash` (generic engine) + `cicd/config.bash` (per-project settings - the only file to edit when copying `cicd/` into a new project).
		- Stages, fail-fast: 1) `cargo fmt` [formats in place before anything compiles]; 2) debug build; 3) `cargo test`; 4) profiler artifact; 5) release builds (native + cross targets); 6) dogfood the native release to a local bin dir; 7) `cicd/utility/n8git_backup-and-publish`. Before running, prints the plan + resolved paths and prompts (`-y` to skip; `--no-fmt/--no-cross/--no-profile/--no-dogfood/--no-publish`). Publish goes hands-off when given `-m/--message MSG` (or `-y`, or `PUBLISH_AUTO_MESSAGE` in config), or when a message is entered at the preflight prompt: a `GIT_EDITOR` helper (`cicd/utility/git-auto-msg.bash`) supplies the commit message so the publish script's `git commit` never opens an editor (a `pull --no-ff` merge message is left intact), and `GIT_BACKUP_AND_PUBLISH_QUIET=1` suppresses its continue-prompt - so the whole run can finish unattended.
		- Profiling is a non-gating artifact, not a pass/fail test. A dev-only, feature-gated `profiling` Cargo feature pulls in `pprof` (in-process SIGPROF sampling - sidesteps `perf_event_paranoid=3`, no sudo, zero shipped-binary cost) plus a `--profile-run` mode (env `SILK_PROFILE_OUT`/`SILK_PROFILE_SECS`) that runs the real app against a heavy unicode workload for N seconds, then writes a flamegraph SVG. Built via `cargo build --profile profiling --features profiling` (the `[profile.profiling]` keeps the symbols/debuginfo that release strips, so frames are named). Output -> `../private/profiling/flame_<stamp>_frequent.svg` (+ stable `flame-latest.svg`), pruned to ~30 by the reusable `gfs_rotate` (first + newest-per-hour/day/week/month/year + last 10). Failure policy: environmental causes (no DISPLAY, missing python3) skip with a warning; a genuine run failure aborts (`PROFILE_STRICT=1` to abort on any). Verified end-to-end: SVG with named frames (`State::render`, `Pane::build`, `TextCtx::fill_glyph`); rotation correct.
	- Old-file rotation is one shared, reusable function (`cicd/utility/include/gfs-rotate.bash`) used for both the profiler flamegraphs and n8git's version backups: keeps about 30 (the first ever, the newest of each recent hour/day/week/month/year, and the last 10), renames kept files to a chronologically-sorting `<prefix>_<YYYYmmDD-HHMMSS>_<role>.<ext>`, tags a period only after it has ended, and conforms any pre-existing files. n8git calls it right after writing a backup (rotation stays its own function, separate from the backup logic).
