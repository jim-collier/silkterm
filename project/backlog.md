...
- 🛠️ nano:
	...
	- nano is not the one emitting both. Driven through a pty at the same size, a run of nothing but up arrows emits `ESC[2;45r` seventeen times and the full-screen restore seventeen times, and never `ESC[2;46r`. So the 1-to-46 region reaching the renderer during an up run comes from our side, and that is where to look first: the engine ledger keeps one region per batch and drops the batch when it changes.
