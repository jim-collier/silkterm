...
- 🛠️ Smooth app-scroll (`smooth_scroll_apps`) left a blank band above/below the text that grew with scroll speed, and stepped one line at a time before easing. (20260703)
	...
	- 🔘 Residual band jitter during a slide (nano; "almost perfect" otherwise). Two symptoms:
		- Text moving UP (content scrolls up): the drop-shadow *under* the inverse-video header title jumps DOWN.
		- Text moving DOWN fast: the bottom two lines jump UP (can't tell if their drop-shadow does too).
		- Diagnosis (needs a live confirm - both are at the band<->scroll-region boundary):
			- The readability glow of the sliding middle region is clipped at the band edge (`top_split_y`/`split_y`), but the blur bleeds across it, and the previous-frame strip that fills the reveal isn't glowed. So the shadow right below a static band is partly contributed by the middle region, which moves - when the slide snaps to its start-of-step offset, that boundary shadow shifts with it. This fits the header-shadow symptom.
			- The bottom-line symptom only shows up "fast enough", which points at re-detection mid-ease: a new step re-captures the band sizes and *sets* `app_off` to a fresh full step, so if the bottom-band count differs for one frame (or `app_off` jumps), the band row shifts by a line for that frame.
		- Likely fix: hold the band sizes stable across an in-progress ease (don't shrink a band mid-slide), and either glow the reveal strip or damp the middle glow near the band edge so the boundary shadow doesn't ride the slide. Feel-sensitive - verify on hardware, gated by `smooth_scroll_apps`.
