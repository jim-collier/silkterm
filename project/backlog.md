...
- 🛠️ Smooth app-scroll (`smooth_scroll_apps`) left a blank band above/below the text that grew with scroll speed, and stepped one line at a time before easing. (20260703)
	...
	- ◐ Residual band jitter during a slide (nano; "almost perfect" otherwise). Two symptoms, different causes:
		- Text moving UP (content scrolls up): the drop-shadow *under* the inverse-video header title jumps DOWN. STILL OPEN (real root cause now known + reproduced).
			...
			- Naive fix attempted + REJECTED (parked on local branch `glowstrip`, do-not-merge): also glow prev_buffer clipped to the strip (mirror the text pass). It fixes the faint strip BUT regresses the header - prev's own header/furniture glyphs, mis-aligned with the current own-bg mask during the sub-cell slide, get an unmasked dark glow and blob the header (mid-slide header dark-px 1130 -> 3742). 
			- Proper fix (TODO, needs care + owner eye): glow the strip from prev but exclude prev's static bands (header/status) so only prev's scrolling content is glowed, and/or inset the prev-glow clip below the header band by ~the glow radius so its blur can't reach the header. Feel-sensitive; gated by `smooth_scroll_apps`.
