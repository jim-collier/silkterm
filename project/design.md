...
Catch-up speed is per burst: an explicit chase speed that starts at the configured initial speed (one line per its milliseconds) and doubles every configured ramp-up period while the backlog stays deep - a visible slow start that ramps to whatever keeps up. A burst whose own first line is still on screen (a short listing) tops out at the configured single-screen speed; once a burst has scrolled a screenful, its first line is provably off the top and the ramp is unlimited. Once output stops running ahead the speed relaxes back to the initial speed over the configured ramp-down. A burst ends when the view settles at the bottom, so sporadic single lines keep the plain initial speed.
...
The six settings that shape all of this are presented in the order they are watched, rather than grouped by mechanism: how gently the view leaves rest (Ease-in), the slow speed it settles into, how hard it accelerates (Ramp-up), the ceiling while the burst still fits on screen, how quickly it relaxes afterwards (Ramp-down), and how gently it lands (Ease-out). Two of those are matched pairs, and it was decided that each pair must run one direction - higher Ease-in and Ease-out are gentler, higher Ramp-up and Ramp-down are harder. That constraint decides how a value is stored rather than the other way round: the landing is stored as how long it takes, not how fast it closes, purely so its slider runs with its partner instead of against it. Storing the mechanism directly would have made one half of each pair read backwards.
