...
- Version-bump guard: the same `pre-push` hook blocks a push to main unless its `source/Cargo.toml` version is a strict increase over the version already on main (full semver precedence, including prerelease ordering) - so a release merge can't ship the same-or-lower version. Skips on the first main push and on branch deletes; overridable with `--no-verify` / `SKIP_GATE=1`.
