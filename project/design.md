...
- What the machine is gets hashed - the processor and its usable core count, the graphics adapter, installed memory to the nearest GiB, and whether the screen is remote - and that hash is what the profile is written down against. The parts that need no adapter are read on a worker at launch, since nothing before the first frame wants them. A different hash is a different machine and gets rated again; the same hash leaves the profile where it was left. One setting on the Silk tab switches the check off for a machine already rated.
- A remote screen goes straight to Standard terminal and is not timed. Every frame is encoded and shipped over a network, so the graphics card says nothing about what the person sees, and a benchmark on it would only flatter the machine. An adapter with no card behind it goes to Low, also untimed. Both answers are decided before anything renders. Because the remote flag is inside the hash, sitting down at the console after a remote session is a different machine and gets its own rating rather than inheriting the remote one.
