...
- A file an earlier build converted with its image written onto the `wallpaper:` heading is repaired where it is at launch: the value moves to `image:` and nothing else in the file changes. It is not converted again, because that needs a backup name, and that build had used most of them converting the same file at every launch. Values that build dropped are not read back from the backups, since a backup is older than anything saved after it.
